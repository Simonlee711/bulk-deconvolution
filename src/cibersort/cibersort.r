...
CIBERSORT <- function(sig_matrix = lm22, mixture_file, perm, QN = TRUE, absolute=False, abs_method='sig.score'){
  ...
  X <- read.csv("./cellanneal/example_data/sc_PBMC_gene_signature.csv")
  rownames(X) <- X$Unnamed: 0
  X$Unnamed: 0 <- NULL
  print(X)
